...
export default function BrowsePage() {
  ...
  const [showReminder, setShowReminder] = useState(
    () => typeof window !== "undefined" && !localStorage.getItem(`reminder_dismissed_${eventSlug}`)
  );
  ...
  useEffect(() => {
    ...
    const CACHE_KEY = `publishers_cache_${eventSlug}`;
    ...
    async function loadPublishers() {
      ...
      try {
        const res = await fetch(`/api/events/${eventSlug}/publishers`);
  ...
  useEffect(() => {
    ...
    async function loadUserData() {
      ...
      try {
        ...
        const [{ data: { session } }, { data: sels, error: selsError }, { data: counts, error: countsError }] = await Promise.all([
          ...
          supabase.rpc("get_my_book_counts", { p_event_id: eventId }),
        ]);
    ...
    loadUserData();
  }, [isLoggedIn, userRetryKey, eventId]);
  ...
  const doRemoveOrAdd = useCallback(async (publisherId: string, isSelected: boolean) => {
    ...
    try {
      ...
      if (isSelected) {
        ...
        const { error } = await supabase.from("user_selections").insert({ user_id: userId!, publisher_id: publisherId, event_id: eventId });
        if (error) throw new Error("insert failed");

        // Auto-join event on first selection
        await supabase.from("user_events").upsert(
          { user_id: userId!, event_id: eventId!, is_active: true },
  ...
  }, [userId, selectedIds, eventId]); // toastTimer is a stable ref — intentionally not listed
  ...
  if (eventError || !eventId) return <div>Event not found</div>;
      ...
      {toast && (
        <div className="fixed left-[16px] right-[16px] bottom-[84px] z-20 animate-[toast-in_0.25s_ease-out]">
          <div className="flex items-center gap-[12px] h-[61px] px-[16px] bg-[#f0e4d4] border border-[#c4855a] rounded-[8px]">
            ...
            <Link href={`/events/${eventSlug}/my-list`} onClick={() => setToast(null)} className="shrink-0">
