...
export default function EventMyListPage() {
  ...
  useEffect(() => {
    ...
    if (process.env.NODE_ENV === "development" && env.NEXT_PUBLIC_DEV_BYPASS_AUTH === "true") {
      setUserId("dev-user");
      setPublishers([
        { id: "p1", name_th: "สำนักพิมพ์แสงดาว", name_en: "Sangdao Publishing", event_id: eventId, booths: [{ zone: "A", booth_number: "A01" }] },
        { id: "p2", name_th: "นานมีบุ๊คส์", name_en: "Nanmeebooks", event_id: eventId, booths: [{ zone: "B", booth_number: "B12" }] },
        { id: "p3", name_th: "อมรินทร์", name_en: "Amarin", event_id: eventId, booths: [{ zone: "C", booth_number: "C05" }] },
      ]);
      setBooks([
        { id: "b1", publisher_id: "p1", title: "ดินแดนแห่งความฝัน", price: 280, is_purchased: false, event_id: eventId },
        { id: "b2", publisher_id: "p1", title: "มหาสมุทรและหุบเขาเล็กๆ", price: 320, is_purchased: true, event_id: eventId },
        { id: "b3", publisher_id: "p2", title: "Harry Potter", price: 450, is_purchased: false, event_id: eventId },
      ]);
      ...
      try {
        ...
        const [{ data: { session } }, { data: sels, error: selsError }, { data: bookData, error: bookError }] = await Promise.all([
          ...
          supabase
            ...
            .eq("event_id", eventId)
            .order("created_at"),
          supabase.from("user_books").select("id, publisher_id, title, price, is_purchased, event_id").eq("event_id", eventId).order("created_at"),
        ]);
  ...
  function removePublisher(publisherId: string) {
    ...
    showToast(
      ...
      async () => {
        const supabase = getSupabase();
        await Promise.all([
          supabase.from("user_selections").delete().eq("user_id", userId).eq("publisher_id", publisherId).eq("event_id", eventId!),
          supabase.from("user_books").delete().eq("user_id", userId).eq("publisher_id", publisherId).eq("event_id", eventId!),
        ]);
      }
    );
  }
  ...
  async function handleNoteSaved(publisherId: string, note: string | null) {
    ...
    if (!(process.env.NODE_ENV === "development" && env.NEXT_PUBLIC_DEV_BYPASS_AUTH === "true") && userId) {
      const supabase = getSupabase();
      await supabase.from("user_selections").update({ note }).eq("user_id", userId).eq("publisher_id", publisherId).eq("event_id", eventId!);
    }
  }
  ...
  async function handleShare() {
    ...
    try {
      const supabase = getSupabase();
      const { data, error } = await supabase.rpc("get_share_stats", { p_event_id: eventId });
  ...
  if (authLoading || eventLoading || loading) return <LoadingScreen />;
  if (eventError) return <ErrorScreen onRetry={() => router.replace(`/events`)} />;
